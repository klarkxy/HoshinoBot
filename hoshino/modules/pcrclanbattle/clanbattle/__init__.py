...
@on_command('#帮助', aliases=('#幫助', '#help'), only_to_me=False)
async def cb_help(session:CommandSession):
    await session.send(QUICK_START, at_sender=True)
    msg = MessageSegment.share(url='https://github.com/klarkxy/HoshinoBot/blob/master/hoshino/modules/pcrclanbattle/clanbattle/README.md',
                               title='Hoshino会战管理v2',
                               content='命令一览表')
